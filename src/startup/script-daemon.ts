...
async function setupEnvironment(ns: NS): Promise<void> {
    ...
    ns.ps(machine.hostname)
        .filter((x) => x.filename !== (ns.getRunningScript() as RunningScript).filename)
        .forEach((script) => ns.kill(script.pid));
    ns.tail();

    singleScripts = [
        { name: "/data/data-writer-daemon.js", runs: [{ args: [], bonusArgs: [], condition: async () => true }] },
        { name: "/startup/file-cleanup.js", runs: [{ args: [], bonusArgs: [], condition: async () => true }] },
    ];

    repeatScripts = [
        {
            name: "/sleeves/sleeve-daemon.js",
            runs: [
                {
        ...
        {
            name: "/singularity/task-daemon.js",
            runs: [
                {
                    args: [],
                    bonusArgs: [],
        ...
        {
            name: "/singularity/crime-committer.js",
            runs: [
                {
                    args: ["--money"],
                    bonusArgs: [],
                    condition: async () => player.bitnodeN !== 8 && machine.ram.max >= 32 && machine.ram.max < 64,
                },
                {
                    args: ["--karma", "--goal", 100],
                    bonusArgs: [],
                    condition: async () => player.bitnodeN === 2 && machine.ram.max >= 64 && player.karma > -100,
                },
                {
                    args: ["--karma", "--goal", 54000],
                    bonusArgs: [],
                    condition: async () => player.bitnodeN !== 2 && machine.ram.max >= 64 && player.karma > -54000,
                },
            ],
        },
        {
            name: "/coding-contracts/contract-solver-daemon.js",
            runs: [
                {
                    args: [],
                    bonusArgs: [],
        ...
        {
            name: "/stock-market/stock-market-daemon.js",
            runs: [
                {
                    args: [],
                    bonusArgs: [],
        ...
        {
            name: "/hacknet/hashnet-server-daemon.js",
            runs: [
                {
                    args: ["--hash-improve", "--hash-hacking", "--hash-bladeburner", "--hash-corp"],
                    bonusArgs: [{ args: ["--hash-no-money", "--wild"], condition: () => hasPublicCorp() }],
                    condition: async () => player.bitnodeN !== 8 && machine.ram.max >= 64 && multipliers.HacknetNodeMoney >= 0.1,
                },
            ],
        },
        {
            name: "/singularity/backdoor-daemon.js",
            runs: [
                {
                    args: ["--all-servers"],
                    bonusArgs: [],
        ...
        {
            name: "/bladeburner/bladeburner-daemon.js",
            runs: [
                {
                    args: [],
                    bonusArgs: [],
        ...
        {
            name: "/gangs/gang-daemon.js",
            runs: [
                {
                    args: [],
                    bonusArgs: [{ args: ["--wild"], condition: async () => hasPublicCorp() }],
                    condition: async () => machine.ram.max >= 64 && ((await playerInGang()) || (player.bitnodeN === 2 && player.karma < -100) || player.karma <= -54000),
                },
            ],
        },
        {
            name: "/staneks-gift/gift-constructor-daemon.js",
            runs: [
                // Start of game --- NOT 8
                {
                    args: ["--crime-money"],
                    bonusArgs: [],
                    condition: async () => player.bitnodeN !== 8 && machine.ram.max >= 128 && !(await playerInGang()) && stanekLargerThan(2, 3),
                },
                // All game -- ONLY 8
                {
                    ...
                    condition: async () => player.bitnodeN === 8 && machine.ram.max >= 128 && stanekLargerThan(2, 3),
                },
                // Post gang-creation -- NOT 6, 7 OR 8 -- HACKING IS GOOD
                {
                    ...
                    bonusArgs: [
                        {
                            ...
                            condition: async () => multipliers.HacknetNodeMoney >= 0.33 && stanekLargerThan(4, 5),
                        },
                        {
                            args: ["--hacking-skill"],
                    ...
                    condition: async () => ![6, 7, 8].includes(player.bitnodeN) && machine.ram.max >= 128 && (await playerInGang()) && stanekLargerThan(2, 3),
                },
                // Post gang-creation -- 6 AND 7
                {
                    ...
                    condition: async () => [6, 7].includes(player.bitnodeN) && machine.ram.max >= 128 && (await playerInGang()) && stanekLargerThan(2, 3),
                },
            ],
        },
        {
            name: "/staneks-gift/gift-charger-daemon.js",
            runs: [
                {
                    args: ["--reserved-ram", 50],
                    bonusArgs: [],
                    condition: async () => (await runDodgerScript<unknown[]>(ns, "/staneks-gift/dodger/activeFragments.js")).length > 0 && machine.ram.max <= 2048,
                },
                {
        ...
        {
            name: "/servers/server-purchase-daemon.js",
            runs: [
                {
                    args: [],
                    bonusArgs: [{ args: ["--wild"], condition: async () => hasPublicCorp() }],
                    condition: async () => serverLimit > 0 && machine.ram.max >= 128 && player.money >= 25e6,
                },
            ],
        },
        {
            name: "/corporation/corporation-daemon.js",
            runs: [
                {
                    args: [],
                    bonusArgs: [],
                    condition: async () => machine.ram.max >= 2048 && multipliers.CorporationValuation >= 0.2 && (player.hasCorp || player.money >= 300e9),
                },
            ],
        },
        {
            name: "/singularity/ascension-daemon.js",
            runs: [
                {
                    args: [],
                    bonusArgs: [
                        {
                            ...
                            condition: async () => {
                                ...
                                return (totalWorth ? totalWorth >= 500e12 : false) && lastAug >= 1000 * 60 * 10;
                            },
                        },
                    ],
        ...
        {
            name: "/singularity/faction-invite-daemon.js",
            runs: [
                {
                    args: [],
                    bonusArgs: [],
        ...
        {
            name: "/hacking/hack-daemon.js",
            runs: [
                {
                    args: [],
                    bonusArgs: [],
                    condition: async () => player.bitnodeN !== 8 && machine.ram.max >= 128 && machine.ram.max < 65536,
                },
                {
                    args: ["--stock-mode"],
                    bonusArgs: [],
                    condition: async () => machine.ram.max >= 65536 && hackingGoal < 3000 * multipliers.WorldDaemonDifficulty * 2,
                },
                {
                    args: ["--xp-farm-mode"],
                    bonusArgs: [],
...
function tryRunScript(ns: NS, script: string, args: string[]): void {
...
function isScriptAlreadyRunning(ns: NS, script: string, args: string[]): boolean {
...
function doRunScript(ns: NS, script: string, args: (string | number)[]): void {
    ...
    if (successfulRun) {
        logger.log(`Started script: ${script} with args: [${args}]`, {
...
async function processScriptRun(ns: NS, script: string, run: IAutoScriptCondition): Promise<void> {
    ...
    const trueArgs = [...run.args, ...bonusArgs].map((x) => (typeof x === 'string' ? x : x.toString()));

    if (await run.condition()) {
        tryRunScript(ns, script, trueArgs);
    } else {
        killOldScriptInstances(ns, script, trueArgs);
    }
}

function killOldScriptInstances(ns: NS, script: string, args: string[]): void {
    const oldInstances = ns
        .ps()
        .filter((proc) => proc.filename === script && proc.args.every((arg) => args.includes(arg)) && args.every((arg) => proc.args.includes(arg)));
    if (oldInstances.length > 0) {
        const instance = oldInstances[0];
        logger.log(`Killing old instance of script: ${instance.filename} with args: [${instance.args}]`, {
