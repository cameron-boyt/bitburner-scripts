...
function hasHackingStats(aug: IAugmentInfo): boolean {
    return !(
        !aug.stats.hacking_chance_mult &&
        !aug.stats.hacking_exp_mult &&
        !aug.stats.hacking_grow_mult &&
        !aug.stats.hacking_money_mult &&
        !aug.stats.hacking_mult &&
        !aug.stats.hacking_speed_mult
    );
}
...
function hasCombatStats(aug: IAugmentInfo): boolean {
    return !(
        !aug.stats.agility_exp_mult &&
        !aug.stats.agility_mult &&
        !aug.stats.defense_exp_mult &&
        !aug.stats.defense_mult &&
        !aug.stats.dexterity_exp_mult &&
        !aug.stats.dexterity_mult &&
        !aug.stats.strength_exp_mult &&
        !aug.stats.strength_mult
    );
}
...
function hasHacknetStats(aug: IAugmentInfo): boolean {
    return !(
        !aug.stats.hacknet_node_core_cost_mult &&
        !aug.stats.hacknet_node_level_cost_mult &&
        !aug.stats.hacknet_node_money_mult &&
        !aug.stats.hacknet_node_purchase_cost_mult &&
        !aug.stats.hacknet_node_ram_cost_mult
    );
}
...
function hasMoneyStats(aug: IAugmentInfo): boolean {
    return !(!aug.stats.work_money_mult && !aug.stats.crime_money_mult && !aug.stats.hacking_money_mult && !aug.stats.hacknet_node_money_mult);
}
...
async function processKarmaRequirement(ns: NS, requirements: IFactionReqs): Promise<boolean> {
    ...
    while (player.karma < requirements.karma) {
        if (!ns.isRunning("/singularity/crime-committer.js", "home", "--karma", "--goal", `${requirements.karma}`)) {
            while (machine.ram.free < ns.getScriptRam("/singularity/crime-committer.js")) {
...
async function processCompanyReputationRequirement(ns: NS, requirements: IFactionReqs): Promise<boolean> {
    ...
    if (companyInfo) {
        ...
        if (!currentPosition) {
            ...
            if (startPosition) {
                ...
                if (startPosition.levelReq) {
                    if (!checkLevelRequirements(ns, startPosition.levelReq, companyInfo.levelOffset)) {
                        return false;
                    }
                }
        ...
        while (ns.getCompanyRep(company) < rep) {
            ...
            logger.log(
                `Processing requirements to receive promotion to: ${nextPositionTitle}\n` + `        | ${Math.round(ns.getCompanyRep(company))} / ${rep} company reputation`,
...
async function getDesireableAugmentations(ns: NS): Promise<IAugmentInfo[]> {
    ...
    allAugs.forEach((aug) => {
        aug.factions = aug.factions.sort(
            (a, b) => allAugs.filter((x) => x.factions.includes(b) && isDesiredAug(x)).length - allAugs.filter((x) => x.factions.includes(a) && isDesiredAug(x)).length
        );
    });
    ...
    for (const faction of ALL_FACTIONS.filter((x) => x !== gangFaction && !(inCityFaction && CITY_FACTIONS.includes(x)))) {
        ...
        const desirableAugsFromFaction = augsFromFaction.filter(
            (aug) =>
                isDesiredAug(aug) &&
        ...
        if (desirableAugsFromFaction.length > 0) {
            ...
            desiredAugs.push(...desirableAugsFromFaction.filter((aug) => desiredAugs.findIndex((x) => x.name === aug.name) < 0 && aug.repReq <= repForNextFavourThreshold));
