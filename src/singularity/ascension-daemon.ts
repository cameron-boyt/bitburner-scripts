...
async function updateAugmentBasket(ns: NS): Promise<IAugmentPurchase[]> {
    ...
    augmentations = await readAugmentData(ns);
    const factionRep = await runDodgerScript<number[]>(ns, "/singularity/dodger/getFactionRep-bulk.js", player.factions.joinedFactions);
    ...
    for (let i = 0; i < player.factions.joinedFactions.length; i++) {
        const faction = player.factions.joinedFactions[i];
        const rep = factionRep[i];
        const availableAugs = augmentations
            .filter((aug) => aug.factions.includes(faction))
            .filter((aug) => aug.name !== "NeuroFlux Governor" && !ownedAugs.includes(aug.name) && rep >= aug.repReq && totalFunds >= aug.cost);
    ...
    if (allAugs.length > 0) {
        ...
        while (purchaseableAugs.length > 0) {
            ...
            purchaseableAugs.sort((a, b) => b.cost - a.cost);
            const mostExpensiveAug = purchaseableAugs[0];
    ...
    const repFactions: { faction: string; rep: number }[] = [];
    for (let i = 0; i < player.factions.joinedFactions.length; i++) {
        repFactions.push({ faction: player.factions.joinedFactions[i], rep: factionRep[i] });
    }

    const gangData = peekPort<IGangData>(ns, PortNumber.GangData);
    const gangFaction = gangData ? gangData.gangInfo.faction : "";

    // Get faction with highest reputation
    const factions = repFactions.filter((fac) => fac.faction !== gangFaction);
    ...
    if (factions.length > 0) {
        ...
        if (!neuroFluxAug) throw new Error("Where is NeuroFlux??");
...
async function purchaseAugsAndReset(ns: NS, basket: IAugmentPurchase[]): Promise<void> {
    ...
    for (const aug of basket) {
        ns.singularity.purchaseAugmentation(aug.faction, aug.augment);
    }

    ns.ps()
        .filter((proc) => proc.filename !== ns.getRunningScript()?.filename)
