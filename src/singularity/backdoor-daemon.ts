...
function getBackdoorableServer(ns: NS): IServerObject[] {
    return getAllServers(ns)
        .filter((x) => !ns.getServer(x).purchasedByPlayer && x != "home")
        ...
        if (!target.isBackdoorInstalled) {
            if (target.hasRootAccess) {
                if (player.stats.hacking >= target.hackLevel) {
                    ...
                    logger.log(`Insufficient hacking level to install backdoor on ${target.hostname} (${player.stats.hacking} / ${target.hackLevel})`, {
