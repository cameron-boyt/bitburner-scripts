 ...
 */
export async function getBestCrime(
    ns: NS,
    type: CrimeType,
    stats: PlayerSkills | SleeveSkills,
    successMult = 1,
    successThreshold = 0,
    ...
    const crimes = crimeData.filter((crime) => calculateCrimeChance(crime, stats, successMult) >= successThreshold && crime.time <= timeThreshold * 1000);
...
function calculateCrimeReturn(type: CrimeType, crime: CrimeStats, stats: PlayerSkills | SleeveSkills, successMult: number): number {
    switch (type) {
        case CrimeType.Money:
            return (calculateCrimeChance(crime, stats, successMult) * crime.money) / crime.time;
        case CrimeType.Karma:
            return (calculateCrimeChance(crime, stats, successMult) * crime.karma) / crime.time;
        case CrimeType.Kills:
            return (calculateCrimeChance(crime, stats, successMult) * crime.kills) / crime.time;
    }
}
