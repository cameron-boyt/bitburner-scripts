...
function getRamUpgradePurchaseAction(ns: NS, index: number): IHashPurchase {
    const stats = hacknetData.servers[index];
    const currentProd = stats.production;
    const newProd = ns.formulas.hacknetServers.hashGainRate(
        stats.level,
        stats.ramUsed,
        Math.pow(2, Math.log(stats.ram) / Math.log(2) + 1),
        stats.cores,
        player.hacknet.productionMult
    );
    const prodIncrease = newProd - currentProd;
    ...
    if (action) {
        logger.log(
            `Action --> ${getActionFromEnum(action.type)} ${action.type === HashAction.BuyNewNode ? "" : "for server " + action.node + " "}` +
                `costing ${ns.nFormat(action.cost, "$0.000a")}`,
            { type: MessageType.debugLow }
        );
    ...
    if (canPurchaseUpgrade(action.cost)) {
        switch (action.type) {
            case HashAction.BuyNewNode:
                return tryDoBuyNewServer(ns, action);
            case HashAction.UpgradeCores:
                return tryDoUpgradeServerCores(ns, action);
            case HashAction.UpgradeLevel:
                return tryDoUpgradeServerLevel(ns, action);
            case HashAction.UpgradeRAM:
                return tryDoUpgradeServerRam(ns, action);
        }
    } else {
...
function canPurchaseUpgrade(cost: number): boolean {
    return player.money >= cost && (wildSpending ? player.money - cost >= 100e9 : hacknetData.currentFunds - cost >= maximumDebt);
}
...
function trySpendHashesOnUpgrade(ns: NS, upgrade: HashUpgrades): boolean {
    ...
    const result =
        upgrade === HashUpgrades.IncreaseMaxMoney || upgrade === HashUpgrades.ReduceMinSecurity
