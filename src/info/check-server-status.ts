    ...
    while (true) {
        ...
        for (const t of hackable_targets) {
            ...
            for (const s of getAllServers(ns).filter((x) => ns.hasRootAccess(x))) {
                if (ns.ps(s).filter((x) => x.filename === "/hacking/single/hack.js" && x.args[0] === t.hostname).length > 0) {
                    hack = true;
                }
                if (ns.ps(s).filter((x) => x.filename === "/hacking/single/grow.js" && x.args[0] === t.hostname).length > 0) {
                    grow = true;
                }
                if (ns.ps(s).filter((x) => x.filename === "/hacking/single/weak.js" && x.args[0] === t.hostname).length > 0) {
                    weak = true;
                }
            }
            ...
            if (t_str.length < host_len) {
                t_str = " ".repeat(host_len - t_str.length) + t_str;
            }

            if (m_str_1.length < mon_len) {
                m_str_1 = " ".repeat(mon_len - m_str_1.length) + m_str_1;
            }

            if (m_str_2.length < mon_len) {
                m_str_2 = m_str_2 + " ".repeat(mon_len - m_str_2.length);
            }

            if (s_str_1.length < sec_len) {
                s_str_1 = " ".repeat(sec_len - s_str_1.length) + s_str_1;
            }

            if (s_str_2.length < sec_len) {
                s_str_2 = s_str_2 + " ".repeat(sec_len - s_str_2.length);
            }
