...
import { IBoardLayout, IFragmentPlacement } from "./staneks-gift-data";
...
function setupEnvironment(ns: NS): void {
    fragmentRotations = [];
    ns.stanek.fragmentDefinitions().forEach((frag) => (fragmentRotations[frag.id] = calculateShapeRotations(frag)));
...
async function constructStanekGiftBoard(ns: NS): Promise<void> {
    ...
    logger.log(`Gift Settings: Height: ${height}, Width: ${width}, Fragments: ${fragmentIdsToPlace}`, { type: MessageType.info });
    ...
    if (loadedBoard) {
        ...
        const tempFragments = await getFragmentPlacements(ns, height, width, []);
        ...
        const optimisedFragments = await getFragmentPlacements(ns, height, width, boosterFragments);
 ...
 */
async function getOptimalFragmentPlacements(
    ns: NS,
    board: number[][],
    depth: number,
    placements: IFragmentPlacement[],
    fragmentsToPlace: Fragment[],
    boosterFragments: Fragment[],
    alt = false
): Promise<IFragmentPlacement[] | undefined> {
    await ns.asleep(1);
            ...
            for (let y = 0; y < board.length - fragHeight; y++) {
                for (let x = 0; x < board[0].length - fragWidth; x++) {
                    ...
                    if (mockCanPlaceFragment(board, x, y, shape)) {
                        ...
                        const deepPlacements = await getOptimalFragmentPlacements(
                            ns,
                            newBoard,
                            depth + 1,
                            [...placements, { fragment: frag, rootX: x, rootY: y, rotation: rot }],
                            fragmentsToPlace,
                            boosterFragments,
                            alt
                        );
...
function scoreBoard(board: number[][], placements: IFragmentPlacement[], alt = false): number {
    ...
    let score = 0;
    const hitBoard = Array(board.length)
        .fill([])
    ...
    for (let y = 0; y < board.length; y++) {
        for (let x = 0; x < board[0].length; x++) {
            if (board[y][x] < 100 && board[y][x] !== -1) {
                if (y > 0 ? board[y - 1][x] === -1 : false) {
                    maxBlob = Math.max(maxBlob, getConnectedFree(board, x, y - 1).length);
                }
                if (y < board.length - 1 ? board[y + 1][x] === -1 : false) {
                    maxBlob = Math.max(maxBlob, getConnectedFree(board, x, y + 1).length);
                }
                if (x > 0 ? board[y][x - 1] === -1 : false) {
                    maxBlob = Math.max(maxBlob, getConnectedFree(board, x - 1, y).length);
                }
                if (x < board[0].length - 1 ? board[y][x + 1] === -1 : false) {
                    maxBlob = Math.max(maxBlob, getConnectedFree(board, x + 1, y).length);
                }
            }
        }
