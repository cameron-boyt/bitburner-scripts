...
function getContractSolution(type: string, data: never): unknown {
    switch (type) {
        case "Find Largest Prime Factor":
            return solveFindLargestPrimeFactor(data as number);
        case "Subarray with Maximum Sum":
            return solveSubarrayWithMaximumSum(data as number[]);
        case "Total Ways to Sum":
            return solveTotalWaysToSum(data as number);
        case "Generate IP Addresses":
            return solveGenerateIPAddresses(data as string);
        case "Merge Overlapping Intervals":
            return solveMergeOverlappingIntervals(data as number[][]);
        case "Algorithmic Stock Trader I":
            return solveAlgorithmicStockTrader(1, data as number[]);
        case "Algorithmic Stock Trader II":
            return solveAlgorithmicStockTrader(1000, data as number[]);
        case "Algorithmic Stock Trader III":
            return solveAlgorithmicStockTrader(2, data as number[]);
        case "Algorithmic Stock Trader IV":
            return solveAlgorithmicStockTrader(data[0] as number, data[1] as number[]);
        case "Unique Paths in a Grid I":
            return solveUniquePathsInAGrid(Array(data[0] as number).fill(Array(data[1] as number).fill(0)));
        case "Unique Paths in a Grid II":
            return solveUniquePathsInAGrid(data as number[][]);
        case "Spiralize Matrix":
            return solveSpiraliseMatrix(data as number[][]);
        case "Find All Valid Math Expressions":
            return solveFindAllValidMathExpressions(data[0] as string, data[1] as number);
